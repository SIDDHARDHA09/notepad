...
const BookmarksPage: React.FC<BookmarksPageProps> = ({ bookmarks, availableTags, onAdd, onUpdate, onDelete }) => {
  ...
  const BookmarkCard = ({ bookmark: b }: { bookmark: Bookmark; key?: React.Key }) => (
    <div key={b.id} className={`premium-card group rounded-[2.5rem] p-10 flex flex-col h-full relative overflow-hidden transition-all duration-500 border ${b.isPinned ? 'border-indigo-500/30 ring-1 ring-indigo-500/10 bg-indigo-50/5 dark:bg-indigo-900/5' : 'border-slate-200/50 dark:border-slate-800/50'}`}>
      ...
      <div className="flex justify-between items-start mb-8 relative z-10">
        <div className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-3xl flex items-center justify-center transition-transform group-hover:rotate-12 duration-500 shadow-sm">
        ...
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
          <button onClick={() => onUpdate(b.id, { isPinned: !b.isPinned })} className={`p-2.5 rounded-xl transition-all hover:scale-110 active:scale-90 ${b.isPinned ? 'text-indigo-600 bg-white dark:bg-slate-800 shadow-sm' : 'text-slate-300 hover:text-indigo-500'}`}><svg className="w-5 h-5" fill={b.isPinned ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg></button>
          <button onClick={() => onUpdate(b.id, { isFavorite: !b.isFavorite })} className={`p-2.5 rounded-xl transition-all hover:scale-110 active:scale-90 ${b.isFavorite ? 'text-rose-500 bg-white dark:bg-slate-800 shadow-sm' : 'text-slate-300 hover:text-rose-500'}`}><svg className="w-5 h-5" fill={b.isFavorite ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg></button>
        </div>
      </div>
      
      <a href={b.url} target="_blank" rel="noopener noreferrer" className="font-black text-2xl text-slate-900 dark:text-white mb-4 line-clamp-2 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors tracking-tight relative z-10 leading-tight">{b.title}</a>
      <p className="text-slate-500 dark:text-slate-400 text-base leading-relaxed line-clamp-3 mb-10 font-medium relative z-10">{b.description}</p>
    ...
    <div className="px-6 lg:px-16 py-12 lg:py-24 space-y-24 animate-in fade-in duration-700">
      <section className="space-y-16">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
          <div className="max-w-3xl space-y-6">
            <h2 className="text-6xl md:text-8xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">Capture the <br/><span className="text-indigo-600 drop-shadow-sm">infinite web.</span></h2>
            <p className="text-xl md:text-2xl text-slate-500 dark:text-slate-400 font-medium leading-relaxed max-w-xl">A curated collection of resources, inspiration, and tools for your digital journey.</p>
          </div>
          <button 
      ...
      {pinnedBookmarks.length > 0 && (
        <section className="space-y-12 animate-in slide-in-from-bottom-12 duration-700">
          <div className="flex items-center gap-5">
            ...
            <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-widest">Essential Links</h3>
          </div>
      ...
      <section className="space-y-12 animate-in slide-in-from-bottom-24 duration-1000">
        <div className="flex items-center gap-5">
          ...
          <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-widest">Web Archives</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          ...
          {filteredBookmarks.length === 0 && (
            <div className="col-span-full py-48 flex flex-col items-center justify-center text-center space-y-10">
              ...
              <div className="space-y-3">
                <h4 className="text-3xl font-black text-slate-800 dark:text-slate-200 tracking-tight">The archive is empty</h4>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 lg:p-12">
          <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-3xl" onClick={() => setIsModalOpen(false)}></div>
          <div className="bg-white dark:bg-slate-900 rounded-[4rem] w-full max-w-xl relative shadow-2xl animate-in zoom-in slide-in-from-bottom-12 duration-700 overflow-hidden border border-white/5">
            <form onSubmit={handleSubmit} className="p-12 lg:p-16">
              <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-16 tracking-tighter leading-none">{editingBookmark ? 'Update Resource' : 'Archive New Link'}</h2>
              <div className="space-y-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Resource URL</label>
                  <div className="relative group">
                    <input required type="url" value={formUrl} onChange={(e) => setFormUrl(e.target.value)} onBlur={handleUrlBlur} className="w-full px-10 py-6 bg-slate-50 dark:bg-slate-800/50 border-none rounded-[2rem] focus:ring-4 focus:ring-indigo-500/20 text-slate-800 dark:text-slate-100 font-bold outline-none transition-all placeholder:text-slate-300" placeholder="https://mindvault.app/resource" />
                    {isFetching && <div className="absolute right-8 top-1/2 -translate-y-1/2"><div className="w-6 h-6 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin"></div></div>}
                  </div>
                </div>
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Display Title</label>
                  <input required type="text" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} className="w-full px-10 py-6 bg-slate-50 dark:bg-slate-800/50 border-none rounded-[2rem] focus:ring-4 focus:ring-indigo-500/20 text-slate-800 dark:text-slate-100 font-bold outline-none transition-all placeholder:text-slate-300" placeholder="Web Page Title" />
                </div>
                <div className="space-y-4">
                ...
                <div className="flex gap-6 pt-12">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-6 bg-slate-100 dark:bg-slate-800 rounded-[2rem] font-black text-slate-600 dark:text-slate-400 hover:scale-105 active:scale-95 transition-all text-sm uppercase tracking-widest">Cancel</button>
                  <button type="submit" className="flex-1 py-6 bg-indigo-600 text-white rounded-[2rem] font-black shadow-2xl shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all text-sm uppercase tracking-widest">Confirm Archive</button>
                </div>
              </div>
            </form>
          </div>
